...
public class DBUtils extends MedSavantServerUnicastRemoteObject implements DBUtilsAdapter {
    ...
    public int getNumRecordsInTable(String sessID, String tablename) throws SQLException, SessionExpiredException {
        try {
            return (isVariantsTable(tablename)) ? shardedHelper.getNumRecordsInTable(CustomTables.getInstance().getCustomTableSchema(sessID, tablename)) : unshardedHelper
                    .getNumRecordsInTable(sessID, tablename);
    ...
    public List<String> getDistinctValuesForColumn(String sessID, String tableName, String colName, boolean explodeCommaSeparatedValues, boolean cacheing)
        ...
        if (isVariantsTable(table.getTableName())) {
            List<Object> temp = shardedHelper.getDistinctValuesForColumn(table, colName, (cacheing ? DistinctValuesCache.CACHE_LIMIT : -1));
    ...
    public Range getExtremeValuesForColumn(String sessID, String tabName, String colName) throws InterruptedException, SQLException, RemoteException, SessionExpiredException {
        ...
        Range result = (isVariantsTable(table.getTableName())) ? shardedHelper.getExtremeValuesForColumn(table, colName) : unshardedHelper.getExtremeValuesForColumn(sessID, query);
