...
public class LoginPanel extends JPanel implements Listener<LoginEvent> {
    ...
    private void initView() {
        ...
        MigLayout ml = new MigLayout("fillx, filly, align center");
        this.setLayout(ml);

        JPanel container = new RoundedPanel(30);
        container.setLayout(new MigLayout("align center"));

        JPanel loginForm = getLoginForm();

        container.add(logo,"wrap, align center");
        container.add(initLogin());
    ...
    private JPanel initLogin() {
        ...
        loginOnEnter(userField);
        userField.setPlaceholder("username");
        userField.setFont(new Font("Helvetica",Font.PLAIN,20));
        passwordField = new PlaceHolderPasswordField();
        loginOnEnter(passwordField);
        passwordField.setPlaceholder("password");
        sizeField(passwordField);
