...
/**
 * A controller for a form.  A form is a collection of CustomFields together with
 * their constraints.  This class registers customFields and constraints, and provides
 * methods for communicating these to the view (e.g. FormEditorDialog).  
 * 
 * For each form, this class should be extended to define the submitForm dialog and 
 * setup any required constraints.
 * 
 * @see CustomField
...
public abstract class FormController { 
    ...
    private String autoIncKey;
     ...
     /**
     * Constructs a new form controller with the given fields.  "AutoIncrement"
     * fields should be included in 'fields', and also specified in autoIncKey.
     * 
    ...
    public final boolean isValid(CustomField cf, String val, String[] err){
       CustomFieldConstraint cfc = constraints.get(cf.getColumnName());
        if(cfc != null){            
    ...
    /**
     * @return The value/label pairs allowed by the enum constraint defined on the field 'cf', or null if 
     * the field has no enum constraint.  e.g. {{Value,Label},{Value,Label},...}
     */
    public final String[][] allowedValues(CustomField cf){                   
        if(enumColumns != null && enumColumns.contains(cf.getColumnName())){
            EnumConstraint ec = (EnumConstraint)constraints.get(cf.getColumnName());
    ...
    public boolean isAutoInc(CustomField f){
        if(autoIncKey != null && autoIncKey.equalsIgnoreCase(f.getColumnName())){
            return true;
        }
        return false;
    }
    ...
    /**
     * Sets up a default constraint on the given field with a generic error message, according to that field's type.  
     * For example, boolean values are constrained to 0 or 1, dates are constrained to conform to YYYY-MM-DD format, etc.
     *      
     */
    protected final void addDefaultConstraint(CustomField cf){   
        ...
        switch(cf.getColumnType()){
            case BOOLEAN:                
                ...
                setConstraint(cf.getColumnName(), new EnumConstraint(new String[][]{{"1", "True"}, {"0", "False"}}, cf.getColumnLength(), errMsg));                
                break;
            case INTEGER:
                errMsg = cn+ " is not a valid integer";
                setConstraint(cf.getColumnName(), new RegexpConstraint(RegexpConstraint.REGEXP_DEFAULT_INTEGER, cf.getColumnLength(), errMsg));                
                break;
            case DATE:
                errMsg = cn+ " is not a valid date";
                setConstraint(cf.getColumnName(), new RegexpConstraint(RegexpConstraint.REGEXP_DEFAULT_DATE, cf.getColumnLength(), errMsg));                                
                break;
            case DECIMAL:              
                errMsg = cn+" is not a valid decimal value";
                setConstraint(cf.getColumnName(), new RegexpConstraint(RegexpConstraint.REGEXP_DEFAULT_DECIMAL, cf.getColumnLength(), errMsg));                                                
                break;
            case FLOAT:
                errMsg = cn+" is not a valid decimal value";
                setConstraint(cf.getColumnName(), new RegexpConstraint(RegexpConstraint.REGEXP_DEFAULT_DECIMAL, cf.getColumnLength(), errMsg));                                                
            ...
            case VARCHAR:
                ...
                setConstraint(cf.getColumnName(), new RegexpConstraint(RegexpConstraint.REGEXP_DEFAULT_VARCHAR, cf.getColumnLength(), errMsg));                                                
                break;
            case TEXT:
                errMsg = cn+" is not a valid string";
                setConstraint(cf.getColumnName(), new RegexpConstraint(RegexpConstraint.REGEXP_DEFAULT_VARCHAR, cf.getColumnLength(), errMsg));                                                
    ...
    /**
     * Convenience method that adds or modifies constraints for all of the given fields so that missing values
     * are no longer allowed for that field.
     */
    ...
    /**
     * Convenience method that adds or modifies constraints for the given fields so that missing values are
     * no longer allowed for that field.
     */
    protected final void setRequiredField(CustomField cf){
        String columnName = cf.getColumnName();
        CustomFieldConstraint cfc = constraints.get(columnName);
        if(cfc != null){
            cfc.setRequired(true);
        }else{            
            cfc = new CustomFieldConstraint(true, cf.getColumnLength());
        }        
        constraints.put(columnName, cfc); 
    }
