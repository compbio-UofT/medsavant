...
public class VariantAnnotator extends MedSavantServerJob implements BasicVariantColumns {
    ...
    public VariantAnnotator(String sessID, MedSavantServerJob parentJob, File inputFile, File outFile, Annotation[] annotations, CustomField[] customFields) throws FileNotFoundException, IOException {
        super(SessionManager.getInstance().getUserForSession(sessID), "VariantAnnotator", (parentJob!=null) ? parentJob.getJobProgressMonitor() : null);
    ...
    public boolean run() {
        ...
        try {
            ...
            getJobProgressMonitor().setMessage("Making room for annotations...");
            //add custom fields
            ...
            if (annotations.length > 0) {
                ...
                BatchVariantAnnotator bva = new BatchVariantAnnotator(this.getJobProgressMonitor(), new File(workingFilePath), new File(annotatedFilename), annotations, sessID);
