...
/**
 *
 * IOjobs submitted through this class respect MAX_IO_JOBS -- there can never be
 * more than MAX_IO_JOBS threads with a running IOJob. If the limit is exceeded,
 * the other IOJobs will wait until an IO permit becomes available. The 'doIO'
 * method of an IOJob is executed a maximum of DEFAULT_NUM_BLOCKS_PER_PERMIT
 * before the permit is temporarily released to other scheduled IO jobs. This
 * default can be overridden with an argument to requestIO.
 */
public class MedSavantIOController {
    ...
    private static int maxJobs = 1;
    private static Semaphore IOSem = new Semaphore(1, true);
    private static final Semaphore IOSemLock = new Semaphore(1, true);

    public static void setMaxIOJobs(final int newMaxJobs) {
        MedSavantServerJob msj
                = new MedSavantServerJob("System", "foo", null) {
                    @Override
                    public boolean run() throws Exception {
                        //make sure several threads aren't trying to setMaxIOJobs simultaneously
                        IOSemLock.acquire();

                        //Block until all current IO jobs are done.
                        IOSem.acquire(maxJobs);

                        //Adjust the maximum number of jobs
                        maxJobs = newMaxJobs;
                        IOSem = new Semaphore(newMaxJobs, true);
                        IOSemLock.release();
                        return true;
                    }
                };
