...
/**
 * This class encapsulates information about a single transcript. Jannovar has
 * functionality to use the data from the UCSC database (Known Genes) to extract
 * information about transcripts. In this case, each line of the UCSC
 * KnownGene.txt file will correspond to one {@code TranscriptModel} object. See
 * {@link jannovar.io.UCSCKGParser} for an explanation of the structure of
 * individual lines. Note that for now, we are not including scaffolds such as
 * chr4_ctg9_hap1 in the parsed lines (they throw an
 * {@link jannovar.exception.KGParseException} and are discarded by
 ...
 * <P>
 * This class provides methods that allow the Chromosome class to calculate what
 * annotations are appropriate for a given variant, and thus represents one of
 * the core classes of Jannovar.
 *
...
public class TranscriptModel implements java.io.Serializable, Constants {
    ...
    /**
     * Accession number of the transcript (e.g., the UCSC knownGene id -
     * uc011nca.2). The version number may be included.
     */
    private String accession = null;
    /**
     * Gene symbol of the known Gene. Can be null for some genes. Note that in
     * annovar, $name2 corresponds to the geneSymbol if available, otherwise the
     * kgID is used.
     */
    private String geneSymbol = null;
    /**
     * Chromosome. chr1...chr22 are 1..22, chrX=23, chrY=24, mito=25. Ignore
     * other chromosomes. TODO. Add more flexible way of dealing with scaffolds
     * etc.
     */
    ...
    private int txEnd;
    /**
     * CDS start position of gene (chromosomal coordinate; Note that
     * {@link #rcdsStart} is the CDS start within the transcript.
     */
    ...
    /**
     * Total length of coding sequence (CDS). Note that if CDS==0, then this is
     * a non-coding gene.
     */
    ...
    /**
     * The Gene id that corresponds to the transcript model. Note that this
     * information is taken from knownToLocusLink.txt or modified Ensembl Gene
     * ids.
     */
    ...
    /**
     * The constructor is private to prevent accidental initialization of an
     * empty TranscriptModel object. The way to make TranscriptModels is to use
     * the static method {@link #createTranscriptModel} and then to set the
     * variables.
     *
    ...
    /**
     * @param c The chromosome on which this gene is located. It is denoted by a
     * byte constant that is defined in
    ...
    /**
     * @param st The start position of the CDS on the chromosome. (Note that
     * {@link #rcdsStart} is the CDS start within the transcript).
     */
    ...
    /**
     * This method causes the TranscriptModel object to initialize various
     * internal variables such as mRNA length following the initial construction
     * of the object. This function was defined during refactoring efforts,
     * placing the parse functions in the IO hierarchy. TODO, is there are more
     ...
     * Note that if an exonEnd is 20 and the exon start is 10 then we have a
     * total of 11 nucleotides, thus the calculation is end-start+1 for each
     * exon.
     */
    ...
    /**
     * Calculate the position of the CDS start (i.e., the start codon) within
     * the entire transcript, essentially equal to the length of the 5' UTR plus
     * one. If the 5' UTR contains one or more introns, then we compensate for
     * this by the cumlenintron calculation (see the code). <br>
     * Due to the format restrictions of the Ensembl GFF format, the position of
     * the CDS start can also be a negative value, if the starting exon is
     * completely spanned by CDS area and there is still a phase offset in the
     * GFF definition.
     * <P>
     * Note that we calculate the rcdslength when we find the exon which it is
     * contained in. We know this because
    ...
    /**
     * This functionality appears in several forms in annovar. Essentially, we
     * want to use the information contained in the gene model to calculate the
     * start position of the variant within the coding sequence. It seems easier
     * to concentrate that functionality in this class rather than in client
     * code.
     * <P>
     * The basic functionality depends on the intron and exons lengths and the
     * exon in which the variant is found. Note in Annovar we have two variants
     * <UL>
     * <LI>{@code  rvarstart = kgl.getExonStart(k)-txstart-cumlenintron+1;}. This
     * is for variants located at least partially in the flanking intron
     * sequence, we take the first exon position)
     * <LI>
     * <P>
     *
     * @param varstart The start position of the variant on the chromosome (can
     * be the actual start position or the start of the exon for variants that
     * are )
     * @param cumlenintron The cumulative length of then intron sequences (This
     * has been calculated in
     * {@link jannovar.reference.Chromosome#getPlusStrandAnnotation getPlusStrandAnnotation}
    ...
    /**
     * Returns the cDNA sequence belonging to this {@link TranscriptModel} as
     * {@link String}. Returns the entire cDNA sequence, not just the ORF.
     *
    ...
    /**
     * Returns the entire sequence of the mRNA starting with the stop codon. It
     * is useful to have this because some deletion mutations extend the mutant
     * coding sequence past the original stop codon.
     *
     ...
     * has been calculated in
     * {@link jannovar.reference.Chromosome#getPlusStrandAnnotation getPlusStrandAnnotation}
     * or the corresponding function for the negative strand). TODO FOr minus
     * strand!
    ...
    /**
     * Checks is the position passed to the function lies 3' to the gene on the
     * chromosome.
     *
    ...
    /**
     * Checks is the position passed to the function lies 5' to the gene on the
     * chromosome.
     *
     * @param pos position of the variant along the current chromosome
    ...
    /**
     * Calculates whether the position given by {@code pos} is 5' to the start
     * of the gene (i.e., txStart) and also within {@code threshold} of the 5'
     * end (txStart) of the gene.
     *
     * @param pos position of variant (end) on current chromosome
     * @param threshold Threshold distance to be considered upstream/downstream
     * (set in Chromosome, should be 1000 nt)
     * @return true if variant is extragenic but within threshold of 5' end of
     * gene (i.e., txStart)
     */
    ...
    /**
     * Calculates whether the position given by {@code pos} is 3' to the end of
     * the gene (i.e., txEnd) and also within {@code threshold} of the 3' end
     * (txEnd) of the gene.
     *
    ...
    /**
     * Calculates the length of the coding sequence based on the exon
     * starts/stop. The logic is as follows
     * <OL>
     * <LI>If the cdsStart lies within [exonStarts[i]..exonEnd[i]] for exon i
     * then the coding sequence begins in this exon
     * <LI>If additionally, the cdsEnd lies in the same exon, then the gene has
     * only one exon, and we calculate the CDS length as cdsEnd - cdsStart + 1.
     * Otherwise, this is the first of multiple exons, and we add the length of
     * the CDS of the first coding exon (exon i) as this.exonEnds[i] - cdsStart
     * + 1. Note that this if clause is the first one that will increment the
     * variable CDSlength, so that the other clauses can only be true once we
     * have already seen the first coding exon.
     * <LI>If cdsEnd is less than exonEnds[i], then we are in the last coding
     * exon, and we need to add the length of the coding segment, which is
     * cdsEnd - exonStarts[i] + 1. We can then skip any remaining exons (break).
     * <LI>If we have already seen the first coding exon, and cdsEnd is larger
     * than exonEnds[i], then it is an internal exon and we can add its entire
     * length as exonEnds[i] - exonStarts[i] + 1.
     * </OL>
     */
    private void calculateCDSLength() throws JannovarException {
        ...
        for (int i = 0; i < this.exonCount; ++i) {
            ...
            if (CDSlength > 0 && cdsEnd < exonStarts[i]) {
                String msg = "Impossible parsing scenario for " + this.accession + " (CDSend is less than exon start)";
                LOG.error(msg);
    ...
    /**
     * If this gene is not coding, then cdsStart is one more than cdsEnd. (Note
     * that in UCSC files, for noncodiing genes cdsstart==cdsend. In our
     * implementation, to change to one-based fully close numeration, we
     * increment cdsstart by one. Therefore, for noncoding genes cdsstart ==
     * cdsend + 1. This is not the case for coding genes).
     *
    ...
    /**
     * If this gene is not coding, then cdsEnd is one more than cdsStart. (Note
     * that in UCSC files, for noncodiing genes cdsstart==cdsend. In our
     * implementation, to change to one-based fully close numeration, we
     * increment cdsstart by one. Therefore, for noncoding genes cdsstart ==
     * cdsend + 1. This is not the case for coding genes).
    ...
    /**
     * @return the chromosomal coordinate of the transcription start of this
     * gene (for genes on + strand) or the transcription end (for genes on -
     * strand).
     */
    ...
    /**
     * @return the chromosomal coordinate of the transcription end of this gene
     * (for genes on + strand) or the transcription start (for genes on -
     * strand).
     */
    public int getTXEnd() {
        return this.txEnd;
    ...
    /**
     * @return the chromosomal coordinate of the start of the coding sequence of
     * this gene (for genes on + strand) or the end of the CDS (for genes on -
     * strand).
     */
    ...
    /**
     * @return the chromosomal coordinate of the end of the coding sequence of
     * this gene (for genes on + strand) or the start of the CDS (for genes on -
     * strand).
     */
    ...
    /**
     * Return length of the actual cDNA sequence (rather than the length
     * calculated from the exon positions, which should however be the same. Can
    ...
    /**
     * Return position of CDS (start codon) in entire mRNA transcript. for
     * transcripts on the minus strand, the corresponding position is calculated
     * by {@link #calculateRefCDSStart} .
     *
    ...
    /**
     * @return genesymbol of this transcript (if available, otherwise the
     * accession number).
     */
    ...
    /**
     * This function is valid for exonic variants. It extracts the three
     * nucleotides from the reference sequence that contain the first nucleotide
     * of the position of the variant. Note that refvarstart is one-based
     * numbering.
     *
     * @param refvarstart Position of first nucleotide of variant in cDNA
     * sequence
    ...
    /**
     * This function is valid for exonic variants. It extracts the three
     * nucleotides from the reference sequence that are directly 3' to the codon
     * that contains the first nucleotide of the position of the variant. If
     * that was the last codon, the return ""; the empty string.
     * <P>
     *
     * @param refvarstart Position of first nucleotide of variant in cDNA
     * sequence
     ...
     * Note that intron 1 begins after exon 1, so there are n-1 introns in a
     * gene with n exons.
     * <P>
     * Note that because exonEnds and exonStarts are both one-based, we return
     * start[k]-end[k-1] -1 as the total length.
     *
     * @param k number of intron (zero-based) whose length is to be sought
     * @return length of the k<superscript>th</superscript> intron (returns zero
     * if k is 0)
     */
    ...
    /**
     * Calculates the length of exon k. Note that we assume that k is a valid
     * exon count (zero-based).
     *
     * The chromosomal positions themselves are one-based fully closed, so that
     * the length of an exon is end-start+1.
     *
     * @param k number of exon (zero-based) whose length is to be calculated.
     * @return length of exon in nucleotides.
     * @see jannovar.reference.Chromosome Chromosome (this class makes use of
     * this method)
     */
     ...
     *
     * @return an integer array with all end positions of the exons of this
     * knowngene
     */
    ...
    /**
     * @return an integer array with all start positions of the exons of this
     * transcript. Note that for genes on the minus strand, the order is
     * "reversed" with respect to the exon counts of the transcript.
     */
    ...
    /**
     * This method is used by {@link jannovar.io.UCSCKGParser UCSCKGParser} to
     * add sequence data (from knownGeneMrna.txt) to the KnownGene object.
     *
    ...
    /**
     * This method is used by {@link jannovar.io.UCSCKGParser UCSCKGParser} to
     * add the Gene id (from knowntoLocusLink.txt) to the KnownGene object.
     *
     * @param id an Gene id
     */
     ...
     * the UCSC kgXref.txt file. By comparing the ucscid, we identify the
    ...
    /**
     * This method can be used during development to print all the data
     * contained in this transcript. In contrast to the method
     * {@link #debugPrintCDS}, it prints out the entire transcript sequence and
     * not just the coding sequence (CDS).
     */
     ...
     * This method can be used during development to print all the data
     * contained in this transcript. In contrast to the method
     * {@link #debugPrint}, it prints out the coding sequence (CDS) and not the
     * entire transcript sequence.
     */
    ...
    /**
     * @return a String representation of the chromosome on which the transcript
     * is located, e.g., chr4 or chrX
     */
     ...
     *
     * @param pos The coordinate of interest. Should be in the exonic region of
     * the transcriptmodel.
     * @return The distance to the CDS start position (without intronic
     * regions).
     */
    ...
    /**
     * Returns the distance to the Start position of the CDS or '-1' if the
     * given coordinate is located in inter- or intragenic region or the gene is
     * noncoding.
     *
     * @param pos The coordinate of interest. Should be in the exonic region of
     * the transcriptmodel.
    ...
    /**
     * Returns the distance to the end position of the CDS or '-1' if the give
     * coordinate is located in inter- or intragenic region or the gene is
     * noncoding.
     *
     * @param pos The coordinate of interest. Should be in the exonic region of
     * the transcriptmodel.
    ...
    /**
     * Returns the mRNA length between the two positions given in chromosomal
     * coordinates. First it is checked that both positions are located in
     * exonic regions of this {@link TranscriptModel}.
     *
    ...
    /**
     * Returns the corresponding chromosomal coordinates for the cDNA start and
     * end position given with <code>start</code> and <code>end</code>.<br>
     * If the start and end coordinates are spanning an intron border multiple
     * start and end coordinates are given, where the first pair is from start
     * to the border, ...<br>
    ...
    /**
     * Returns the position on the cDNA string for a given chromosomal position
     * or -1 if the chromosomal position is not exonic.
     *
