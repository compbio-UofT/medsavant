...
/**
 * This class encapsulates a chromosome and all of the genes its contains. It is
 * intended to be used together with the
 * {@link jannovar.reference.TranscriptModel TranscriptModel} class to make a
 * list of gene models that will be used to annotate chromosomal variants. We
 * use an {@link jannovar.interval.IntervalTree IntervalTree} to store all of
 * the {@link jannovar.reference.TranscriptModel TranscriptModel} objects that
 * belong to this Chromosome and to search for all transcripts that overlap with
 * any given variant. Note that the IntervalTree class has functionality also to
 * find the neighbors (5' and 3') of the closest gene in order to find the right
 * and left genes of intergenic variants and to find the correct gene in the
 * cases of complex regions of the chromosome with one gene located in the
 * intron of the next or with overlapping genes.
 * <P>
 * Note that the {@link jannovar.interval.Interval Interval} objects in the
 * interval tree are defined by the transcription start and stop sites of the
 * isoform.
 *
...
public class Chromosome {
    ...
    /**
     * Alternative String for Chromosome. Use for scaffolds and "random"
     * chromosomes. TODO: Refactor
     */
    private final String chromosomeString = null;
    /**
     * Total number of TranscriptModels on the chromosome including multiple
     * transcripts of the same gene.
     */
    private final int n_genes;
    /**
     * The initial capacity for
     * {@link jannovar.reference.TranscriptModel TranscriptModel} objects for
     * the
     *
    ...
    /**
     * The distance threshold in nucleotides for calling a variant
     * upstream/downstream to a gene,
     */
    ...
    /**
     * This object will be used to prioritize the annotations and to choose the
     * one(s) to report. For instance, if we have both an intronic and a
     * nonsense mutation, just report the nonsense mutation. Note that the
     * object will be initialized once in the constructor of the Chromosome
     * class and will be reset for each new annotation, rather than creating a
     * new object for each variation. Also note that the constructor takes an
     * integer value with which the lists of potential annotations get
     * initialized. We will take 2*SPAN because this is the maximum number of
     * annotations any variant can get with this program.
     */
    private AnnotatedVariantFactory annovarFactory = null;

    /**
     * An {@link jannovar.interval.IntervalTree IntervalTree} that contains all
     * of the {@link jannovar.reference.TranscriptModel TranscriptModel} objects
     * for transcripts located on this chromosome.
     */
    private IntervalTree<TranscriptModel> itree = null;

    /**
     * The constructor expects to get a byte representing 1..22 or
     * 23=X_CHROMSOME, or 24=Y_CHROMOSOME (see
     * {@link jannovar.common.Constants Constants}).
     *
     * @param c the chromosome
     * @param intrvtree An interval tree with all transcripts on this
     * chromosome.
     */
    ...
    /**
     * Main entry point to getting Annovar-type annotations for a variant
     * identified by chromosomal coordinates. When we get to this point, the
     * client code has identified the right chromosome, and we are provided the
     * coordinates on that chromosome.
     * <P>
     * The strategy for finding annotations is based on the perl code in
     * Annovar. Roughly speaking, we identify a list of genes affected by the
     * variant using the interval tree ({@link #itree}). and then annotated the
     * variants accordingly. If no hit is found in the tree, we identify the
     * left and right neighbor and annotate to intergenic, upstream or
     * downstream
     *
     * @param position The start position of the variant on this chromosome
     * (one-based numbering)
     * @param ref String representation of the reference sequence affected by
     * the variant
     * @param alt String representation of the variant (alt) sequence
     * @return a list of {@link jannovar.annotation.Annotation Annotation}
     * objects corresponding to the mutation described by the object (often just
     * one annotation, but potentially multiple ones).
    ...
    /**
     * This function is called when the query position(s) represented by start
     * and end do not overlap with any isoforms. In this case, it must be
     * integenic. UPSTREAM and DOWNSTREAM annotations are special cases of
     * INTERGENIC when the variant is within 1000 nucleotides of a transcript.
     *
     ...
     * @param leftNeighbor The transcript that is closest to the variant on the
     * left
     * @param rightNeighbor The transcript that is closest to the variant on the
     * right
     */
    public void createIntergenicAnnotations(int start, int end, TranscriptModel leftNeighbor, TranscriptModel rightNeighbor) throws JannovarException {
        ...
        if (leftNeighbor != null && leftNeighbor.isNearThreePrimeEnd(start, NEARGENE)) {
            /**
             * The following function creates an upstream or downstream
    ...
    /**
     * Main entry point to getting Annovar-type annotations for a variant
     * identified by chromosomal coordinates for a TranscriptModel that is
     * transcribed from the plus strand. This could theoretically be combined
     * with the Minus strand functionalities, but separating them makes things
    ...
    public void getPlusStrandAnnotation(int position, String ref, String alt, TranscriptModel kgl) throws AnnotationException {
        ...
        for (int k = 0; k < exoncount; ++k) {
            ...
            } /* end; if (start < kgl.getExonStart(k)) */ else if (start <= kgl.getExonEnd(k)) {
                /* ----------------------------------------------------------------------- *
     ...
     * Main entry point to getting Annovar-type annotations for a variant
     * identified by chromosomal coordinates for a TranscriptModel that is
     * transcribed from the minus strand. This could theoretically be combined
     * with the Minus strand functionalities, but separating them makes things
     * easier to comprehend and debug.
     *
     * @param position The start position of the variant on this chromosome
    ...
    public void getMinusStrandAnnotation(int position, String ref, String alt, TranscriptModel kgl) throws AnnotationException {
        ...
        /**
         * *************************************************************************************
         * * Iterate over all exons of the gene. Start with the 3'-most exon,
         * which is the first * exon for genes transcribed from the minus
         * strand. *
         * *************************************************************************************
         */
        for (int k = exoncount - 1; k >= 0; k--) {
            ...
            } /* end; if (end > kgl.getExonEnd(k)) */ else if (end >= kgl.getExonStart(k)) {
     ...
     * {@code sub annotateExonicVariants} { my ($refseqvar, $geneidmap, $cdslen,
     * $mrnalen) = @_; (...)
     * <P>
     * Finally, the $refseqvar in Annovar has the following pieces of
     * information {@code my ($refcdsstart, $refvarstart,
     * $refvarend, $refstrand, $index, $exonpos, $nextline) = @ $refseqvar-> $seqid}->[$i]};}
     * Note that refcdsstart and refstrand are contained in the TranscriptModel
     * objects
     *
     * @param refvarstart The start position of the variant with respect to the
     * CDS of the mRNA
     * @param refvarend The end position of the variant with respect to the CDS
     * of the mRNA
    ...
    private void annotateExonicVariants(int refvarstart, int refvarend, int start, int end, String ref, String var, int exonNumber, TranscriptModel kgl) throws AnnotationException {
        ...
        } /* if (start==end) */ else if (var.equals("-")) {
    ...
    /**
     * Return the reverse complement version of a DNA string in upper case. Note
     * that no checking is done in this code since the parse code checks for
     * valid DNA and upper-cases the input. This code will break if these
    ...
    /**
     * This function constructs a HashMap<Byte,Chromosome> map of Chromosome
     * objects in which the
     * {@link jannovar.reference.TranscriptModel TranscriptModel} objects are
     * entered into an {@link jannovar.interval.IntervalTree IntervalTree} for
     * the appropriate Chromosome.
     *
